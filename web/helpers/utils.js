...
//     }
//
//     return Math.abs(val) < 0.0000000001;
// };
//
...
// };
//
// export const isEqualObject = (obj0, obj1) => {
//     if (obj1 === obj2) {
//         return true;
//     } else {
...
//             .forEach((subpath) => iterator(path.join(filepath, subpath)));
//     };
//
//     iterator(filepath);
//
...
//     const path = _require('path');
//     const basedir = path.join(__dirname, '../..').replace(/^\w:/, '').replace(/\\/gmi, '/');
//     return abspath.replace(/\\/gmi, '/').replace(new RegExp(`^(\\w:)?${basedir}/[^/]+`), '').replace(/\//gmi, path.sep);
// };
//
...
// };
//
// export const fill = (arr = [], val, start, end) => {
//     for (let i = start; i < end; i++) {
//         arr[i] = val;
//     }
//     return arr;
