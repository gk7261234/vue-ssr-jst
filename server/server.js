...
// const http = require('../web/helpers/http');
import http from '../web/helpers/http';
...
for (let pageName in router) {
    ...
    console.log('=============router', pageConfig);
    server[pageConfig.method](pageConfig.url, ((pageName) => {
        if (pageConfig.hasOwnProperty('proxy')) {
            return async (req, res) => {
                console.log('ajax===========>body', req.body);
                const data = await http({
                    method: pageConfig.method,
                    url: pageConfig.proxy,
                    data: {
                        current_page: 1,
                        enddate: "2019-09-24",
